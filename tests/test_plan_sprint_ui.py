...
def test_plan_sprint_detail_has_no_redundant_confirm_all_button(client):
    """"Confirm all & push to Jira" was removed as redundant with
    select-all + "Confirm selected" -- the batch action must still work,
    just via the one remaining path.
    """
    _seed_plan()  # page 1001 is "planned"
    resp = client.get("/ui/plan-sprint/sprint-24")
    assert resp.status_code == 200
    assert "Confirm all" not in resp.text
    assert "Confirm selected &amp; push to Jira" in resp.text
    assert 'id="select-all-planned-checkbox"' in resp.text


def test_plan_sprint_detail_gives_the_no_routing_label_sentinel_a_neutral_treatment(client):
    """jira/sprint_planner.py's fallback for a repo target with no routing
    label configured -- "(no routing label configured -- matches every
    page)" -- is a normal config state (e.g. a single-repo setup), not a
    per-page problem. Must NOT render as a "missing label" warning in any
    of the three places it can appear: label_gap, predicted_labels, or the
    file_changes group heading.
    """
    settings = get_settings("testuser")
    store = SprintPlanStore(settings.sprint_plan_store_path)
    no_label = "(no routing label configured -- matches every page)"
    store.put(
        SprintPlan(
            sprint_tag="sprint-9", gate_label="brd", space_key="SD",
            created_at="2026-01-01T00:00:00+00:00", order=["2001"],
            pages=[
                SprintPlanPage(
                    page_id="2001", page_title="Single-repo feature",
                    page_url="https://example.atlassian.net/wiki/spaces/SD/pages/2001",
                    page_version=1, page_body_html="<p>spec</p>", page_labels=["brd"],
                    previous_version=None, diff_text="(first seen)\n\nspec", is_first_seen=True,
                    body_checksum="abc", predicted_labels=[no_label], applied_labels=[],
                    label_gap=[no_label],
                    depends_on_page_ids=[], dependency_rationale="",
                    file_changes=[{"repo_label": no_label, "file_path": "app.py", "change": "add filter", "is_new_file": False}],
                    phase="planned",
                ),
            ],
        )
    ...
    resp = client.get("/ui/plan-sprint/sprint-9")
    assert resp.status_code == 200
    # The alarming warning treatment must not fire for the sentinel alone.
    assert "missing label" not in resp.text
    assert "&#9888;&#65039;" not in resp.text  # no warning-triangle emoji anywhere on the page
    # Neutral, shorter labels take its place.
    assert "No repo routing configured" in resp.text
    assert "Files changed" in resp.text
    assert no_label in resp.text  # still present, e.g. as a tooltip -- just not as a warning
